...
func InitSysDictionary(db *gorm.DB) {
	...
	*status = true
	Dictionaries := []model.SysDictionary{
		...
		{GeaModel: global.GeaModel{ID: 5, CreatedAt: time.Now(), UpdatedAt: time.Now()}, Name: "数据库字符串", Type: "string", Status: status, Desc: "数据库字符串"},
		{GeaModel: global.GeaModel{ID: 6, CreatedAt: time.Now(), UpdatedAt: time.Now()}, Name: "数据库bool类型", Type: "bool", Status: status, Desc: "数据库bool类型"},
	}
