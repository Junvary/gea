...
func JWTAuth() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if claims.ExpiresAt-time.Now().Unix() < claims.BufferTime {
			claims.ExpiresAt = time.Now().Unix() + global.GeaConfig.JWT.ExpiresTime
			...
			if global.GeaConfig.System.UseMultipoint {
				...
				if err != nil {
					global.GeaLog.Error("get redis jwt failed", zap.Any("err", err))
...
func NewJWT() *JWT {
	return &JWT{
		[]byte(global.GeaConfig.JWT.SigningKey),
	}
}
