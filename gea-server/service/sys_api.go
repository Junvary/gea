...
func CreateApi(api model.SysApi) (err error) {
	if !errors.Is(global.GeaDb.Where("path = ? AND method = ?", api.Path, api.Method).First(&model.SysApi{}).Error, gorm.ErrRecordNotFound) {
		return errors.New("存在相同api")
	}
	return global.GeaDb.Create(&api).Error
}
...
func DeleteApi(api model.SysApi) (err error) {
	err = global.GeaDb.Delete(&api).Error
...
func GetAPIInfoList(api model.SysApi, info request.PageInfo, order string, desc bool) (err error, list interface{}, total int64) {
	...
	db := global.GeaDb.Model(&model.SysApi{})
...
func GetAllApis() (err error, apis []model.SysApi) {
	err = global.GeaDb.Find(&apis).Error
	return
}
...
func GetApiById(id float64) (err error, api model.SysApi) {
	err = global.GeaDb.Where("id = ?", id).First(&api).Error
	return
}
...
func UpdateApi(api model.SysApi) (err error) {
	...
	err = global.GeaDb.Where("id = ?", api.ID).First(&oldA).Error
	...
	if err != nil {
		...
		if err != nil {
			...
			err = global.GeaDb.Save(&api).Error
		}
	}
	return err
}
